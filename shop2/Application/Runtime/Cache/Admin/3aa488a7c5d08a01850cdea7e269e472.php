  ...
  <div id="brand_style">
    <div class="search_style">
     ...
     <div class="border clearfix">
       <span class="l_f">
        <a href="<?php echo U('Brand/add');?>"  title="添加品牌" class="btn btn-warning Order_form"><i class="icon-plus"></i>添加属性</a>
        <a href="<?php echo U('Brand/test', ['id' => 1]);?>" class="btn btn-danger"><i class="icon-trash"></i>批量删除</a>
       </span>

      <?php if($count < 1): ?><span class="r_f" style="font-size:17px; color:red;">暂无数据, 请添加属性~</span>
      <?php else: ?>
          <span class="r_f">共：<b style="color:red; font-size:16px;"  id="count_id"></b>&nbsp;个品牌&nbsp;&nbsp;&nbsp;共启用： <b style="color:green; font-size:16px;" id="showCount_id"></b>&nbsp;个</span><?php endif; ?>
...
<?php if(is_array($data)): foreach($data as $key=>$vo): ?><tr>
          ...
          <td class="text-l"><?php echo ($vo["size"]); ?></td>
          <td><?php echo ($vo["num"]); ?></td>
<td></td>
          <td class="td-status">
            <?php if(($vo['status'] == null)): ?><span class="label label-success radius">已启用</span>
