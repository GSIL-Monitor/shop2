...
<?php  foreach($one as $v): ?>
        <dl class="permission-list">
            <dt><label class="middle"><input name="rule[]" class="ace" type="checkbox"  id="id-disable-check" <?php if($v['id']==32) { echo 'checked';}?>    value="<?php echo ($v['id']); ?>"><span class="lbl"><?php echo ($v['title']); ?></span></label></dt>
            <dd>
            <?php  foreach($two as $v2): if($v2['pid']==$v['id']){?> 
                <dl class=" permission-list2" style="height:45px">
                    <dt><label class="middle"><input type="checkbox" value="<?=$v2['id']?>" <?php if($v2['id']==39) { echo 'checked';}?>  class="ace"  id="user-Character-0-1" name="rule[]" ><span class="lbl"><?=$v2['title']?></span></label></dt>
                <dd>
            <?php  foreach($three as $v3): if(!empty($v3[$v2['id']])) {?>
            
                    <label><input type="checkbox" value="<?=$v3['0']?>" <?php if($v3['0']==37 || $v3['0']==38 || $v3['0']==46 ) { echo 'checked';}?>  class="ace" name="rule[]" ><span class="lbl" ><?=$v3[$v2['id']]?></span></label>
