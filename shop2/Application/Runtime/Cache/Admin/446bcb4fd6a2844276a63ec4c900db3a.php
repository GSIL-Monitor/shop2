...
<div class="page-content clearfix">
 ...
 <!--列表样式-->
    ...
    <tbody>
        <?php if(is_array($assesss)): foreach($assesss as $key=>$vo): ?><tr>
       ...
       <td>
          <?PHP  if ($vo['level'] == 1) { echo '好评'; } if ($vo['level'] == 2) { echo '中评'; } if ($vo['level'] == 3) { echo '差评'; } ?>
       </td>
       <td><?php echo ($vo['time']); ?></td>
       <td class="td-status">
        <?php if($vo['status'] == 1): ?><span class="label label-success radius">已通过
        <?php else: ?><span class="label label-defaunt radius">待审核<?php endif; ?>
        </span>

       </td>
      <td class="td-manage">
         <?php if($vo['status'] == 1): ?><a onClick="member_stop(this,<?php echo ($vo["id"]); ?>,<?php echo ($vo["status"]); ?>)"  href="javascript:;" title="屏蔽"  class="btn btn-xs btn-success"><i class="fa fa-check  bigger-120"></i></a>  
            <?php else: ?><a style="text-decoration:none" class="btn btn-xs " onClick="member_start(this,<?php echo ($vo["id"]); ?>,<?php echo ($vo["status"]); ?>)" href="javascript:;" title="通过"><i class="fa fa-close bigger-120"></i></a><?php endif; ?>  
        ...
        <a title="商家评论"  href="<?php echo U('Assess/doReply',['aid'=>$vo['id']]);?>"  class="btn btn-xs btn-info" ><i class="fa fa-edit bigger-120"></i></a>
       </td>
...
function member_stop(obj,id,status){
    layer.confirm('确认要屏蔽吗？',function(index){
       ...
       $.ajax({
            url: "<?php echo U('Assess/editAssessStatus');?>",
            type: "post",
...
function del(obj, id) {
    ...
    layer.confirm('确认要删除吗？',function(index){
        ...
        $.ajax({
            url: "<?php echo U('Assess/delAssessData');?>",
            type: "post",
            data: "id="+id,
            success: function (msg) {
